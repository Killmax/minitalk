...
** Last update Thu Mar 19 18:31:28 2015 bruguet Maxime
*/
...
char		*get_next_line(const int fd)
{
  ...
  if (buf[my_strlen(buf) - rd] == '\0')
    {
      if ((rd = read(fd, buf, 32000)) <= 0)
        return (res = (last-- && buf[my_len(buf) - rd - 1] != 10) ? res : NULL);
      buf[rd] = '\0';
    }
  if ((res = (i == 0) ? malloc(sizeof(*res) * 32000 + 1) :
       my_realloc(res, sizeof(*res) * 32000 + 1)) == NULL)
    return (NULL);
